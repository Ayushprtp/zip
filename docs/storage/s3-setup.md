...
- Pick a region (e.g., `us-east-2`)
  - Dev/Test example: `Flare.sh-dev` (public GET on `uploads/` only if needed)
  - Prod example: `Flare.sh-prod` (private)
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::Flare.sh-dev/uploads/*"
    }
  ]
}
```
