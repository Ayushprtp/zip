    ...
    while (i < input.length) {
      if (state.insideArtifact) {
        ...
        if (state.insideAction) {
          ...
          const { index: actionOpenIndex, tag: matchedActionTag } = this.#findFirstTag(input, ACTION_TAGS, i);
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              // Incomplete action tag, wait for more data
              break;
            }
        ...
        const { index: artifactOpenIndex, tag: matchedArtifactTag } = this.#findFirstTag(input, ARTIFACT_TAGS, i);

        if (artifactOpenIndex !== -1) {
          // Check if it's NOT a close tag that happens to contain the open tag string
          ...
          if (partialMatchIndex !== -1) {
            ...
            i = partialMatchIndex;
            // if we have a partial match, we need to break out of the loop and wait for more data
            break;
          } else {
